...
class Example extends Simulation {
  ...
  val method_2 = tryMax(2){
    ...
    .check(substring("123").find.exists)
    ).pace(5)
    }.exitHereIfFailed
  ...
  val scn = scenario("Condition")
    ...
    .doIfEqualsOrElse("123","124"){exec(Request1.method_1)}{exec(method_2)}
    ...
    .randomSwitch(0.2->exec(Request1.method_1), 0.8->exec(method_2))
    .randomSwitchOrElse(0.2->exec(Request1.method_1), 0.7->exec(method_2)){exec(method_3)}
  ...
  val scn4 = scenario("Rendezvous").rendezVous(100);
    ...
    val scn2 = scenario("loop")
      ...
      .during(30 milliseconds){exec(Request1.method_1)}
      .foreach(ids, "id"){exec(Request1.method_1).exec(http("${id}").get(""))}
      .forever("forever"){exec(Request1.method_1)}
      .asLongAs(isAsLongAs){exec(method_2)}

  // only one group in a single scenario
  var scn3 = scenario("Transaction")
